...
SRC = sys.argv[1]
DST = sys.argv[2]
XML = sys.argv[3] if len(sys.argv) > 3 else "openvino_language_model.xml"
MAXPOS = int(os.environ.get("LUT_MAXPOS", "40960"))  # position_ids are clamped to this

core = ov.Core()
m = core.read_model(os.path.join(SRC, XML))
...
for name in list(byname):
    node = byname[name]
    if node.get_type_name() != "Sin" or ("rotary_emb" not in name and "rope" not in name.lower()):
        continue
    sin_node = node
...
ov.save_model(m, os.path.join(DST, XML), compress_to_fp16=False)

# copy the small sidecar files + link the big untouched bins
import shutil, glob
for f in glob.glob(SRC + "/*"):
    b = os.path.basename(f)
    if b.startswith(XML.rsplit(".", 1)[0] + "."):
        continue
